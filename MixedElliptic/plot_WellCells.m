function varargout = plot_WellCells(Mesh, WellCells, varargin)
% PLOT_MESH Mesh plot.
%
...
if(nargin > 1)
  ...
  % Add labels/flags according to the string OPT
    ...
    % Add well cell labels to element
    Elements = Mesh.Elements(WellCells,:);
    add_WellCellLabels(Mesh.Coordinates,Elements); 
    title(['Well cells']);
...
function [] = add_WellCellLabels(Coordinates,WellElements)
...
%   CH-8092 Zurich, Switzerland
  ...
  [nElements,nVert] = size(WellElements);
  for i = 1:nElements
    CoordMid = sum(Coordinates(WellElements(i,:),:),1)/nVert;
    text(CoordMid(1),CoordMid(2),'WC', ...
