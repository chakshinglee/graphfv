...
for i = 1:Nx*(Ny-1)+(Nx-1)*Ny
    ...
    if numel(AE_no) == 2
        ...
        for k = 1:2
            ...
            velocity_dof_loc_k = find(AE_edge(AE_no(k),:));
            Bloc = A(el_loc+nEdges,velocity_dof_loc_k);
            P = MPp((AE_no(k)-1)*size_para+1:AE_no(k)*size_para);
            P = reshape(P,numel(el_loc),size_para/numel(el_loc));
            V = Bloc'*P;
            snap_marker = ismember(velocity_dof_loc_k,snap_dof_loc);
            assert(norm(velocity_dof_loc_k(snap_marker)-snap_dof_loc)==0,'snap_marker is not correct');
            trace(:,(k-1)*(BasisPerAE)+(1:BasisPerAE)) = V(snap_marker,:);
        end
...
% %         trace = [trace1/2 trace2];
        ...
        [U,S,~] = svd(trace);
%         if sum(diag(S)>1e-8)>(BasisPerEdge)
%             trace = U(:,1:BasisPerEdge);
%         else        
            trace = U(:,diag(S)>1e-8);
%         end
        trace = eye(size(trace,1));
        ...
        Uloc = [0*Uloc_int(1:size_v,:);trace];
...
% Pu = speye(nEdges);
% Pu = spdiags(diag(M).^-1,0,nEdges,nEdges)*B'*Pp;
...
tic;
[voff,doff]=eigs(A_off2,2,eps);
% [doff,I]=sort(diag(doff),'ascend');
Ph_off = Pp*voff(:,2);
fprintf('Coarse problem solved in %f seconds\n',toc);

F_off = P_off'*F;
Uh_off2_tmp = A_off2(2:end,2:end)\F_off(2:(Nx-1)*(Ny-1)*BasisPerAE);
Uh_off2 = -Pp(:,2:end)*Uh_off2_tmp;
Uh_off2 = Uh_off2-sum(Uh_off2)*ones(nElements,1)/nElements;
Uh_off3 = [Pu*tmp(:,2:end)*Uh_off2_tmp;Uh_off2];

VDof = setdiff(1:nEdges,VelocityBndDof(:));
A2 = B(:,VDof)*(M(VDof,VDof)\B(:,VDof)');
tic;
[voff,dd]=eigs(A2,2,eps);
Ph = voff(:,1);
fprintf('Fine problem solved in %f seconds\n',toc);
norm(Ph-Ph_off)/norm(Ph)
norm(Ph+Ph_off)/norm(Ph)
