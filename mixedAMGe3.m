...
for i = 1:Nx*(Ny-1)+(Nx-1)*Ny
    ...
    if numel(AE_no) == 2
        ...
        Floc = zeros(numel(int_dof_loc),2*BasisPerAE-1);
        Floc(size_v+1:end,1) = [ones(nxAE*nyAE,1);-ones(nxAE*nyAE,1)];
        for  j = 1:BasisPerAE-1
            Floc(size_v+1:end,1+j) = [MPp((AE_no(1)-1)*size_para+j*nxAE*nyAE+(1:nxAE*nyAE));zeros(nxAE*nyAE,1)]; 
            Floc(size_v+1:end,BasisPerAE+j) = [zeros(nxAE*nyAE,1);MPp((AE_no(2)-1)*size_para+j*nxAE*nyAE+(1:nxAE*nyAE))];
        end
        Floc = Floc*hx*hy;
        
        ActiveDofLoc = 1:size_v+2*nxAE*nyAE-1;
        Uloc_int = Aloc(ActiveDofLoc,ActiveDofLoc)\Floc(ActiveDofLoc,:);
        Uloc = Uloc_int(1:size_v,:);
        
        trace = Uloc(numel(velocity_int_dof_loc)+1:end,1:end);
        %         trace(:,1) = 1;
...
%         end
        ...
        S = diag(S);
        S = S/max(S);
%         trace = trace*V(:,S>1e-8);
        trace = U(:,S>1e-8);
        if sum(S>1e-8)>BasisPerEdge
            trace = trace(:,1:BasisPerEdge);
        end
%         trace = [const_one/norm(const_one) trace];
...
for i = 1:size(Coordinates2,1)
    for j = 1:size(Coordinates2,2)
        ...
        if sqrt((U12(i,j)^2+U22(i,j)^2))<.005
            U12(i,j) = 0;
            U22(i,j) = 0;
        end
    end
