...
for i = 1:Nx*(Ny-1)+(Nx-1)*Ny
    ...
    if numel(AE_no) == 2
        ...
        trace = zeros(numel(snap_dof_loc),2*(BasisPerAE));
        for k = 1:2
            el_loc = find(AE_el(AE_no(k),:));
            velocity_int_dof_loc = find(AE_int_edge(AE_no(k),:));
            ...
            Am = [hx/hy./coef(el_loc)/2; hx/hy./coef(el_loc)/2; hy/hx./coef(el_loc)/2; hy/hx./coef(el_loc)/2];
            Im = [EdgeLoc(el_loc,1); EdgeLoc(el_loc,2); EdgeLoc(el_loc,3); EdgeLoc(el_loc,4)];
            Jm = [EdgeLoc(el_loc,1); EdgeLoc(el_loc,2); EdgeLoc(el_loc,3); EdgeLoc(el_loc,4)];
            M_tmp = sparse(Im,Jm,Am);
            Mloc = M_tmp(snap_dof_loc,snap_dof_loc);
            
            size_v = numel(velocity_dof_loc);
            Aloc = A(int_dof_loc,int_dof_loc);
            Aloc(1:numel(snap_dof_loc),1:numel(snap_dof_loc)) = Mloc;
            Floc = zeros(numel(int_dof_loc),BasisPerAE);
            %         Floc(size_v+1:end,1) = [ones(nxAE*nyAE,1);-ones(nxAE*nyAE,1)];
            for  j = 1:BasisPerAE
                Floc(size_v+1:end,j) = MPp((AE_no(k)-1)*size_para+(j-1)*nxAE*nyAE+(1:nxAE*nyAE));
            end
            Floc = Floc*hx*hy;
            
            Uloc = Aloc\Floc;
            trace(:,(k-1)*(BasisPerAE)+(1:BasisPerAE)) = Uloc(1:numel(snap_dof_loc),:);
        end
        trace1 = [trace(:,1) trace(:,BasisPerAE+1)];%ones(numel(snap_dof_loc),1)
%         if abs(sum(trace1))<1e-12
%             trace1 = trace(:,1)-trace(:,BasisPerAE+1);
%         end
%         [~,~,V] = svd(trace1);
%         trace1 = trace1*V(:,1);
%         
        trace2 = [trace1 trace(:,2:BasisPerAE) trace(:,BasisPerAE+2:2*BasisPerAE)]; 
        [~,S,V] = svd(trace2);
        S = diag(S);
        S = S/max(S);
...
%         trace = [ones(numel(snap_dof_loc),1) trace(:,1:end)];
...
errL2v = sqrt(diff(1:nEdges)'*A(1:nEdges,1:nEdges)*diff(1:nEdges))/normL2v;
errL2p = norm(diff(nEdges+1:end))*sqrt(hx*hy)/normL2p;
